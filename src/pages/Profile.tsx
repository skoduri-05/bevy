...
const tagLabel = (slug: TagSlug) =>
    TAG_FACETS.flatMap((f) => f.options).find((o) => o.value === slug)?.label ?? slug;
...
export default function Profile() {
    ...
    useEffect(() => {
        ...
        const ids = showPopup === "followers" ? profile.followers ?? [] : profile.following ?? [];
    ...
    const handleCreatePost = async () => {
        ...
        try {
            ...
            if (recipeErr) {
                console.error("Error creating recipe:", recipeErr);
            ...
            if (updErr) {
                console.error("Error updating profile posts:", updErr);
            }
        ...
        try {
            ...
            if (uploadError) {
                ...
                if ((uploadError as any).status === 409) {
                    // conflict: try again with a different name
                    const altPath = `${profile.id}/${timestamp}_${rand}_${Math.floor(Math.random() * 10000)}.${ext}`;
                    ...
                    setUploadError(`Upload failed: ${uploadError.message || "see console for details"}`);
                    return;
                }
            }
    ...
    async function finalizeImageUrl(path: string) {
        try {
            ...
            const { data: publicData } = supabase.storage.from("recipe_images").getPublicUrl(path);
    ...
    if (!profile)
        return (
            <div className="px-4 pb-24">
                <div className="rounded-2xl border border-zinc-700 bg-zinc-900 p-6 text-center max-w-md mx-auto">
                    <h3 className="text-lg font-semibold mb-2">Welcome to Bevy</h3>
                    <p className="text-sm text-zinc-400 mb-4">
                        You’re not signed in yet. Create an account or sign in to view and create posts.
                    </p>
                    ...
                    <div className="flex flex-col gap-2 items-stretch mb-3">
                        <input
                            ...
                            className="w-full px-3 py-2 rounded-xl bg-zinc-800 border border-zinc-700 text-white placeholder-zinc-500"
                        />
                        <button
                            onClick={handleSignIn}
                            disabled={loading || !email}
                            className="px-4 py-2 rounded-xl bg-purple-600 text-white disabled:opacity-50 hover:bg-purple-500 transition"
                        >
                    ...
                    <div className="flex justify-center">
                        <button
                            onClick={() => navigate("/")}
                            className="px-4 py-2 rounded-xl border border-zinc-700 text-white hover:bg-zinc-800 transition"
                        >
                            Browse as guest
                        </button>
                    </div>
    ...
    return (
        <div className="px-4 pb-24">
            ...
            <div className="mt-3 flex gap-4 text-sm">
                <div>
                    <span className="font-semibold text-white">{profile.post_counter ?? 0}</span>{" "}
            ...
            {showPopup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-zinc-900 rounded-2xl p-4 w-80 max-h-[70vh] overflow-y-auto">
                        <h3 className="text-white mb-3">{showPopup === "followers" ? "Followers" : "Following"}</h3>
                        {popupUsers.length > 0 ? (
                            <ul className="space-y-2">
                                {popupUsers.map((u) => (
                                    ...
                                    >
                                        {u.avatar_url && (
                                            <img src={u.avatar_url} alt="avatar" className="h-8 w-8 rounded-full object-cover" />
                                        )}
                                        <div>
                                            <div className="font-medium">{u.name ?? "Unnamed"}</div>
                                            <div className="text-xs text-zinc-400">{u.handle ?? ""}</div>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                                ...
                                {tempTags.length > 0 ? (
                                    tempTags.map((t) => (
                                        <span
                                            ...
                                            onClick={() => setTempTags(tempTags.filter((tag) => tag !== t))}
                                        >
                                            {tagLabel(t)} ×
                                        </span>
                                    ))
                                ...
                                {TAG_FACETS.flatMap((f) => f.options).map((o) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`px-3 py-1 rounded-full border text-xs transition ${selected
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
                    <div className="bg-zinc-800 text-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col gap-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setNewPost({ ...newPost, drink_name: e.target.value })}
                        />
                        <input
                            className="w-full p-2 rounded bg-zinc-700 text-sm"
                            placeholder="Location Purchased"
                            value={newPost.location_purchased}
                            onChange={(e) => setNewPost({ ...newPost, location_purchased: e.target.value })}
                        />
                        <input
                            className="w-full p-2 rounded bg-zinc-700 text-sm"
                            type="number"
                  ...
                  hover:file:bg-zinc-600"
                            ...
                            {newPost.image_url && (
                                <img src={newPost.image_url} alt="Preview" className="mt-2 h-32 w-auto rounded-lg object-cover" />
                            )}
                        </div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2 max-h-32 overflow-y-auto">
                                {availableTags
                                    ...
                                    .map((tag) => (
                                        <button
                                            key={tag}
                                            className={`px-2 py-1 rounded-lg text-xs ${newPost.tags.includes(tag) ? "bg-white text-black" : "bg-zinc-600"
                                                }`}
                                            onClick={() => {
                        ...
                        <div className="mt-auto flex justify-end gap-2">
                            ...
                            <button className="px-3 py-1 rounded-lg bg-white text-black text-sm" onClick={handleCreatePost}>
                                Post
                            </button>
                        </div>
                    </div>
